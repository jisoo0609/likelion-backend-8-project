...
- 필수 과제 구현 완료
- 추가 과제 검색 기능 구현 완료
- 현재 추가과제인 해시태그 부분 개발중이라 해당 코드가 남아있다.
- 게시글 생성 시 html 페이지에 해시태그 입력 칸이 남아 있으나, 해시태그 입력해도 추가되지 않는다.
- 현재 구현이 완료된 기능(필수 과제, 추가과제 - 검색)의 작동 시에는 문제가 없으나, 해시태그 입력 후 엔터키를 누르면 게시글이 작성되니 유의해야 한다.

## 프로젝트 진행 중 에러사항
### 1. 게시글 작성 기능을 구현하는 도중 어려움이 있었다.
- 게시글 작성 시에 게시판을 지정하여 게시글이 작성되어야 한다.
- 하지만, 생성 버튼을 누른 특정 게시판 페이지 내에서만 작성이 되는 오류가 있었다.
- html의 `option`은 제대로 전달되고 있었으나, `Controller`에서 `modelAttribute`값이 제대로 전달되지 않고 있었다.
- `model.addAttribute`에 `option`에서 필요한 `boards`값을 추가했다.

### 2. 프로젝트 초반 `th:` 태그 이용에 낯섦이 있었다.
- html 문서의 낯설음과 `th:`동작의 이해가 낮아, 초반 `th:`구현에서 낯설음이 있었다.
- 이는 프로젝트 진행 과정에서, 반복적인 코드의 사용으로 현재는 초반보다 사용시에 이해도가 높다고 느껴진다.

### 3. 게시글 정렬을 반대로 하고 있었다.
- 테스트하던 와중, 게시판에 정렬되는 게시글이 요구사항과 반대로 정렬되고 있음을 확인했다.
- 요구사항에는 항상 나중에 작성된 게시글이 최상단에 작성되어야 한다고 명시되어 있다.
- 즉, 가장 최근에 작성된 게시글이 최상단에 작성되어야 하는 것인데, 이와 반대로, 가장 최근에 작성된 게시글이 상단에 작성되고 있었다.

### 4. DTO 사용에 대한 어려움이 있었다.
- 처음에 수업 도중 활용했던 방법과 마찬가지로 Entity를 직접 사용해 제작했는데 이후 DTO를 사용했어야 하는것을 발견했다.
- 이에 전면적인 코드 수정은 제출 기한을 맞추지 못할 것 같아 일단 Entity를 사용해 프로그램을 제작했다.
- 이후 리팩토링시에는 DTO를 사용할 수 있도록 수정할 예정이다.

### 5. 프로젝트 완성 후 최종 점검하는 도중 요구사항 누락을 발견했다.
- 게시판의 기능에서 전체 게시글을 위한 전체 게시판이 존재하는 것을 놓쳤다.
- 이를 발견하고 바로 수정해 현재는 전체 게시판의 구현은 되어 있으나, 코드의 리팩토링이 필요하다.
- 현재 프로젝트를 클론하고 실행했을 때에는 제대로 동작하고 화면에서 확인할 수 있으나, `README` 파일의 하단의 이미지는 수정되지 않은 버전이다.
...
- N:M 관계의 Entity를 다루는 것은 처음이라 해당 테이블을 어떻게 관리해야 하는지 더 많은 예제의 참고가 필요할 것 같다.

# 프로젝트 실행 방법
- 해당 레포지토리를 로컬 저장소로 clone해 InteliJ에서 빌드 후 실행한다.
- 별도의 설정이나 설치는 필요하지 않으며, 프로젝트를 실행했을때 웹의 `localhost:8080/boards` 에서 프로그램의 동작을 확인할 수 있다.
- 해당 페이지가 프로젝트의 첫 페이지로 메인 화면이 된다.
- `jpa.hiberante.ddl-auto`의 설정은 `update`로 설정되어 있으며, 해당 프로젝트를 실행했을 때 `db.data`는 `Board`의 `name`만 존재한다.
- 해당 프로젝트의 `README.md`의 내용은 [notion](https://www.notion.so/240108-240111-bf7a82e4986242caa0dbaecdf1979baa?pvs=4)에서도 똑같이 확인할 수 있다.
